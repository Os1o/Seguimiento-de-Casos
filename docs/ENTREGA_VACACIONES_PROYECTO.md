...
Adicionalmente, el fallback público del proyecto ya no conserva credenciales sensibles de base de datos.
La conexión depende del archivo privado configurado fuera del webroot.
...
- si no hay actividad durante 10 minutos, la sesión se cierra automáticamente
- las peticiones autenticadas validan token de sesión, no solo cookie

### Rate limiting en login

Se agregó control de intentos fallidos de inicio de sesión para reducir riesgo de fuerza bruta.

Actualmente se contempla:

- bloqueo por usuario después de varios intentos fallidos
- bloqueo por IP después de un umbral mayor
- bloqueo temporal automático

Los parámetros quedaron configurables desde la configuración del servidor.

### Validación de archivos

La carga de documentos PDF fue reforzada para validar:

- tamaño máximo permitido
- extensión `.pdf`
- MIME detectado del archivo
- firma real de PDF
- nombre seguro al guardar
- validación de archivo subido por mecanismo HTTP real
...
- expiración de sesión por inactividad a 10 minutos
- validación reforzada de token de sesión por request
...
- bloqueo de edición y seguimiento directo en asuntos acumulados, incluso si se intenta entrar por URL
- integración de HTTPS autofirmado
- eliminación de credenciales sensibles hardcodeadas del fallback público
- rate limiting en login
- endurecimiento de validación de uploads PDF en civil y penal
...
- Aún queda como mejora futura una revisión más amplia de salida segura / XSS, aunque no se considera el riesgo más urgente en esta etapa.
...
- mantener activo el rate limiting de login y ajustar umbrales solo si la operación real lo requiere
- dejar para una siguiente fase la revisión de salida segura / XSS en frontend
