...
function requireCivilLawyerReassignmentAccess(array $user, int $delegacionId): void
{
    if (isAdminUser($user)) {
        return;
    }
...
try {
    ...
    $caseStmt = $pdo->prepare('
        ...
        LEFT JOIN usuarios actual
            ON actual.id = ec.abogado_responsable_id
        WHERE ec.id = :id
          AND ec.activo = TRUE
        LIMIT 1
    ');
    ...
    if (!$case) {
        sendError('Asunto no encontrado', 404);
    }

    $delegacionId = isset($case['delegacion_id']) ? (int) $case['delegacion_id'] : 0;
    requireCivilLawyerReassignmentAccess($user, $delegacionId);
    ...
    auditLog($pdo, $user, [
        ...
        'descripcion' => sprintf(
            'Se reasigno el abogado responsable del asunto %s',
            (string) $case['numero_expediente']
        ),
        'detalles' => [
            'cambios' => [
