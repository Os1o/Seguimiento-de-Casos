...
// === UX: Toggle secciones colapsables ===
document.querySelectorAll('.btn-toggle-seccion').forEach(btn => {
    btn.addEventListener('click', function() {
        const content = this.closest('.seccion').querySelector('.seccion-content');
...
// === UX: Textos expansibles ===
document.querySelectorAll('.texto-expansible').forEach(el => {
    el.addEventListener('click', function() {
        this.classList.toggle('expanded');
    });
});

// === UX: Mostrar partes si hay contenido ===
function verificarPartes() {
    const actorInfo = document.getElementById('actorInfo');
    const demandadosInfo = document.getElementById('demandadosInfo');
    const codemandadosInfo = document.getElementById('codemandadosInfo');
    
    if (actorInfo && actorInfo.innerHTML.trim() !== '') {
        document.getElementById('seccionActor').style.display = 'block';
    }
    if (demandadosInfo && demandadosInfo.innerHTML.trim() !== '') {
        document.getElementById('seccionDemandados').style.display = 'block';
    }
    if (codemandadosInfo && codemandadosInfo.innerHTML.trim() !== '') {
        document.getElementById('seccionCodemandados').style.display = 'block';
    }
}

// Ejecutar después de cargar datos
setTimeout(verificarPartes, 100);